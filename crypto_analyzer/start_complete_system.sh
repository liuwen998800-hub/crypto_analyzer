...
# 3. 启动策略API服务（端口5002）
start_service "策略API服务" \
    "python3 src/strategy/strategy_api.py" \
    "logs/strategy_api.log" \
    5002
...
check_service "AI分析系统" "http://localhost:5000/api/status" "/tmp/AI分析系统_pid.txt"
check_service "FMZ集成API" "http://localhost:5001/api/fmz/status" "/tmp/FMZ集成API_pid.txt"
check_service "策略API服务" "http://localhost:5002/api/strategy/health" "/tmp/策略API服务_pid.txt"
...
echo "   测试: curl http://localhost:5000/api/analyze -X POST -H 'Content-Type: application/json' -d '{\"symbol\":\"BTC\"}'"
echo ""
echo -e "${GREEN}✅ FMZ集成API${NC}"
echo "   地址: http://localhost:5001"
echo "   功能: 连接FMZ平台、执行交易信号"
echo "   测试: curl http://localhost:5001/api/fmz/status"
echo ""
echo -e "${GREEN}✅ 策略API服务${NC}"
echo "   地址: http://localhost:5002"
echo "   功能: 策略引擎、交易决策、风险管理"
echo "   测试: curl http://localhost:5002/api/strategy/strategies"
echo ""
echo -e "${YELLOW}📁 日志文件${NC}"
echo "   AI分析系统: logs/ai_system.log"
echo "   FMZ集成API: logs/fmz_api.log"
echo "   策略API服务: logs/strategy_api.log"
...
echo "1. 📊 AI分析系统分析市场数据"
echo "2. 🧠 生成AI交易信号"
echo "3. 🎯 策略引擎处理信号，生成交易决策"
echo "4. ⚡ 策略执行器执行决策"
echo "5. 🔗 FMZ集成API调用FMZ平台"
echo "6. 💰 FMZ平台执行实际交易"
echo "7. 📈 监控交易结果，优化策略"
...
echo "============================================================"
echo "1. 测试完整系统:"
echo "   python3 test_strategy_system.py"
echo ""
echo "2. 测试AI分析:"
...
echo ""
echo "3. 测试策略处理:"
echo "   curl -X POST http://localhost:5002/api/strategy/process-signal \\"
echo "     -H 'Content-Type: application/json' \\"
echo "     -d '{\"signal\":{\"symbol\":\"BTC_USDT\",\"direction\":\"buy\",\"confidence\":75,\"price\":75000},\"strategy\":\"balanced\"}'"
echo ""
echo "4. 测试FMZ集成:"
echo "   curl http://localhost:5001/api/fmz/status"
echo ""
echo "5. 查看策略性能:"
echo "   curl http://localhost:5002/api/strategy/performance"
...
echo "   pkill -f 'python3 src/strategy/strategy_api.py'"
echo ""
echo "或使用:"
echo "   ./stop_complete_system.sh"

# 创建停止脚本
cat > stop_complete_system.sh << 'EOF'
#!/bin/bash
# 停止完整AI量化交易系统
...
# 停止策略API服务
echo "停止策略API服务..."
pkill -f "python3 src/strategy/strategy_api.py"
...
chmod +x stop_complete_system.sh
...
# 等待用户中断
trap 'echo -e "\n${RED}正在停止所有服务...${NC}"; ./stop_complete_system.sh; exit 0' INT

# 显示实时日志（可选）
echo -e "\n${YELLOW}📋 查看实时日志:${NC}"
echo "   终端1: tail -f logs/ai_system.log"
echo "   终端2: tail -f logs/fmz_api.log"
echo "   终端3: tail -f logs/strategy_api.log"
...
while true; do
    ...
    if [ $(date +%M) == "00" ] || [ $(date +%M) == "30" ]; then
        echo -e "${YELLOW}🔄 定期检查服务状态...${NC}"
        for service in "AI分析系统" "FMZ集成API" "策略API服务"; do
            pid_file="/tmp/${service}_pid.txt"
            if [ -f "$pid_file" ]; then
                pid=$(cat "$pid_file")
                if ! ps -p $pid > /dev/null; then
                    echo -e "${RED}⚠️  $service 已停止，尝试重启...${NC}"
                    # 这里可以添加重启逻辑
                fi
            fi
        done
